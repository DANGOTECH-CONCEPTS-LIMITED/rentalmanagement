...
interface NotifItem {
  id: string;
  type: "invoice" | "payment" | "complaint" | "sms" | "viewing" | "system";
  title: string;
  subtitle: string;
  time: string;
  isNew: boolean;
...
const typeIcon: Record<NotifItem["type"], React.ReactNode> = {
  ...
  system: <CalendarClock className="h-4 w-4 text-teal-600" />,
};
...
const isAutoGenerated = (inv: any): boolean => {
  const notes = (inv.notes ?? inv.Notes ?? "").toLowerCase();
  return notes.includes("monthly rent") || notes.includes("system (auto)") || notes.includes("manual trigger");
};
...
const Header = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (role === 2) {
        ...
        // Invoices — auto-generated ones show as "system" type so they stand out
        if (results[1].status === "fulfilled") {
          const invoices: any[] = results[1].value.data ?? [];
          invoices.slice(0, 15).forEach((inv: any) => {
            const t = inv.createdAt ?? inv.CreatedAt;
            const auto = isAutoGenerated(inv);
            const tenantName = inv.tenant?.fullName ?? inv.tenantName ?? "";
            items.push({
              id: `inv-${inv.id}`,
              ...
              subtitle: auto
                ...
                : `${inv.status ?? "Pending"} · UGX ${Number(inv.amount ?? 0).toLocaleString()}`,
              time: t,
              isNew: t ? new Date(t) > lastSeenDate : false,
            });
          });
  ...
  useEffect(() => {
    ...
    // Poll for new notifications every 60 seconds so the landlord sees
    // auto-generated invoices without needing to refresh the page.
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-1">
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                            {n.type === "system" && (
                              <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-teal-100 text-teal-700 leading-none">
                                AUTO
                              </span>
                            )}
                            {n.isNew && (
