...
import { MessageSquare, Search, Filter, Eye, CheckCircle, XCircle, Clock, AlertCircle } from 'lucide-react';
...
const HandleComplaints = () => {
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'high':
        return <span className="px-2 py-1 rounded-full bg-red-100 text-red-800 text-xs">High</span>;
      case 'medium':
        return <span className="px-2 py-1 rounded-full bg-yellow-100 text-yellow-800 text-xs">Medium</span>;
      case 'low':
        return <span className="px-2 py-1 rounded-full bg-green-100 text-green-800 text-xs">Low</span>;
      default:
        return null;
    }
  };
